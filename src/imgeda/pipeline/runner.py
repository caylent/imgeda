...
from imgeda.io.manifest_io import append_records, read_manifest, write_meta
...
def _analyze_wrapper(args: tuple[str, ScanConfig]) -> ImageRecord:
    """Wrapper for ProcessPoolExecutor map."""
    return analyze_image(args[0], args[1])
...
def _run_scan_inner(
    ...
    if config.resume and not config.force and output.exists():
        ...
        if processed_count > 0:
            console.print(
                f"  Resuming: [green]{processed_count:,}[/green] already processed, "
                f"[bold]{len(pending):,}[/bold] remaining"
            )
    else:
        pending = all_images
        # Write fresh manifest header
        meta = ManifestMeta(
    ...
    with progress:
        ...
        with ProcessPoolExecutor(max_workers=workers, initializer=worker_init) as executor:
            futures = {executor.submit(analyze_image, path, config): path for path in pending}
    ...
    # Update meta with final count
    _meta, final_records = read_manifest(output_path)
    if _meta:
        _meta.total_files = len(final_records)
        write_meta(output_path, _meta)

    # Summary
    console.print()
    console.print("[bold green]Scan complete![/bold green]")
    console.print(f"  Total processed: [bold]{len(final_records):,}[/bold]")
    if corrupt_count:
    ...
    return len(final_records), corrupt_count
